...
Object.entries(NETWORKS).forEach(([network, config]) => {
  providers[network] = new ethers.providers.JsonRpcProvider(config.rpc);
...
async function indexCampaignEvents(network, fromBlock, toBlock) {
  ...
  try {
    ...
    for (const event of createdEvents) {
      ...
      await client.query(
        ...
        [
          ...
          ethers.utils.formatUnits(campaign.target, 8),
          campaign.socialLink,
          campaign.imageId.toString(),
          campaign.creator,
          campaign.ended,
          ethers.utils.formatUnits(campaign.totalStable, 8),
          network,
    ...
    for (const event of endedEvents) {
      const campaignId = event.args.campaignId.toString();
      const finalAmount = ethers.utils.formatUnits(event.args.finalStableValue, 8);
...
async function indexDonationEvents(network, fromBlock, toBlock) {
  ...
  try {
    ...
    for (const event of donationEvents) {
      ...
      const amount = ethers.utils.formatUnits(args.netUSDValue, 8);
...
async function indexWithdrawalEvents(network, fromBlock, toBlock) {
  ...
  try {
    ...
    for (const event of requestEvents) {
      ...
      const amount = ethers.utils.formatUnits(event.args.amount, 8);
